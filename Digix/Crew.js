...
async function getUserNumber() {
    return new Promise((resolve) => {
        ...
        rl.question('📲 Enter your WhatsApp number (with country code, e.g., 241xxxx): ', (number) => {
            rl.close();
            resolve(number.trim());
        });
    });
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            try {
                ...
                const messageText = `
╔══════════════════╗
      *ryanxd Bot Connected Successfully* 🚀
╠══════════════════╣
    ...
    setTimeout(async () => {
        if (!state.creds.registered) {
            ...
            try {
                ...
                const code = await sock.requestPairingCode(number, 'RYANDAVE');
                console.log('📲 Pairing Code:', code);
                console.log('👉 Enter this code on your WhatsApp app to pair.');

                setTimeout(() => {
                    configmanager.config.users[number] = {
                        sudoList: ['24176309066@s.whatsapp.net'], // emplace par ton numéro WhatsApp 
